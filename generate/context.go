...
// File returns the name of the file associated with the genCtx context type.
func (ctx *genCtx) File() string {
	return ctx.file
}

// Files returns a slice of strings that contains the names of all the files in
// the file system accessed by genCtx.
func (ctx *genCtx) Files() []string {
	return ctx.files
}

// Identifier returns the identifier of the type or function being generated by
// the generator context [*genCtx].
func (ctx *genCtx) Identifier() string {
	return ctx.identifier
}

// Read reads the contents of the specified file [io.ReadAll()] and returns it
// as a byte slice, caching the result for future calls. If the file has already
// been read and cached, it is returned from the cache instead of being read
// again.
